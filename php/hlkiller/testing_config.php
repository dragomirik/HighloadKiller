        ...
        public static function queries_array($index=null) {
            $arr =array(
                    function() {
                         ...
                         if($likes[0]) {
                             \hlkiller_core::db()->query("DELETE FROM `users_posts_like` WHERE `users_id`='$users[users_id]' AND `posts_id`='$posts[posts_id]'");
                         } else {
                             \hlkiller_core::db()->query("INSERT INTO `users_posts_like` (`users_id`,`posts_id`,`rel_users_posts_like_time`) VALUES ('$users[users_id]','$posts[posts_id]',UNIX_TIMESTAMP())");
                         }
                    },
                    function() {
                        $users_q = \hlkiller_core::db()->query("SELECT `users_id` FROM `users` ORDER BY RAND() LIMIT 1");
                        ...
                        $likes = $likes_q->fetch_array(MYSQLI_NUM);
                        if($likes[0]) {
                            \hlkiller_core::db()->query("DELETE FROM `rel_users_following` WHERE `following_id`='$likes[0]'");
                        } else {
                            \hlkiller_core::db()->query("INSERT INTO `rel_users_following` (`users_follower_id`,`users_supplier_id`,`following_time`) VALUES ('$users[users_id]','$users2[users_id]',UNIX_TIMESTAMP())");
                        }
                    },
                    function() {
                        $users_q = \hlkiller_core::db()->query("SELECT `users_id` FROM `users` ORDER BY RAND() LIMIT 1");
                        ...
                        \hlkiller_core::db()->query("INSERT INTO `rel_users_posts_comments` (`users_id`,`posts_id`,`rel_users_posts_comments_time`,`comments_text`) VALUES ('$users[users_id]','$posts[posts_id]',UNIX_TIMESTAMP(),'$text')");
                    ...
                    function() {
                        ...
                        if($pc_q->num_rows) {
                            \hlkiller_core::db()->query("DELETE FROM `rel_users_following` WHERE `posts_id`='$posts[posts_id]' AND `categories_id`='$categories[categories_id]'");
                        }  else {
                            \hlkiller_core::db()->query("INSERT INTO `rel_posts_categories` (`posts_id`,`categories_id`) VALUES ('$posts[posts_id]','$categories[categories_id]')");
                        }
                    ...
                    function() {
                        ...
                        $time = time();
                        \hlkiller_core::db()->query("INSERT INTO `users` (`users_username`,`users_password`,`users_created_on`) VALUES ('$name','$pass','$time')");
                    }
                ); 
